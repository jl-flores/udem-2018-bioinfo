    ...
    class TimeCounter:
        ...
        def print_progress(self, met_result):
            ...
            if self.count % 500 == 0 or self.count == self.target or self.count == 1:
                with open(f'/u/floresj/Scripts_log/log_distance_file{FILE_NUMBER}.2.txt', 'w') as fp:
                    fp.write(f'Subset\t{FILE_NUMBER}.2\n')
...
#        compteur.print_progress( calculate_similarity(df.loc[ind], apt21_signvec, .05, ind) )
    ...
    similarities.to_csv(f'/u/floresj/Transcriptome_scanning_apta21/Distances/similarities_subset{FILE_NUMBER}.2.csv')